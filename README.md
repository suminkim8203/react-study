...
- 먼저 내용을 정리하고 참조하자.
  : 태그를 먼저 찾고 이벤트를 연결한다.
  : 태그.소문자 on이벤트명 연결한다.
  : on이벤트명 함수 연결시 실행(Call)하면 안 되는 주의사항
  : 여러 개의 함수를 등록할 수 없다.
...
- 아래의 내용을 먼저 정리
  : 리액트는 JSX 버전이 기본
  : 이벤트는 소문자가 아니다
  : 이벤트는 on소문자에 대문자 시작하는 이벤트명
  : 이벤트에 연결된 함수는 두가지 방식으로 작성이 가능하다
  : 함수명()로 ()를 붙이면 함수 Call, 즉 실행되기 때문에 하면 안 됨
  : 함수명만 적어주어야 한다
  : 매개변수 전달되는 함수라면 화살표 함수로 감싸주자
...
```txt
1. 객체 생성 함수와 클래스를 같이 비교하면 이해가 편하다.
...
1. 클래스는 extends와 super 키워드로 상속 가능. 생성자 함수는 extends와 super 키워드를 지원하지 않는다.
    ...
    // constructor? new 붙이면 실행되는 함수



1. 클래스는 호이스팅이 발생하지 않는 것처럼 동작하고,
...
1. 클래스 내부에는 자동으로 strict mode가 지정되며 해제할 수 없다. 생성자 함수는 직접 지정해 줘야한다.

1. 클래스의 constructor, 프로토타입 메서드, 정적 메서드는 모두 열거되지 않는다.(`[[Enumerable]]` ⇒ `false`)

- 클래스에서 정의한 메소드의 특징


class Person{
		// new Person() 하면 자동 실행되는 메서드 : 인스턴스 생성자 함수
	constructor(){
	// return this; // new 해서 만들어진
...
- 접근제한자
  private 속성 : 외부에서 읽기, 쓰기, 변경 안 됨.
  public 속성 : 외부에서 읽기, 쓰기, 변경 다 됨.
  protected 속성 : 허락된 환경만 외부에서 읽기, 쓰기 변경이 됨.
- 접근자 프로퍼티
...
: return 작성하지 않아도 자동으로 기재 된다.
: 아래는 기본형

1. 실행해야 하는 함수 블록이 한 줄이라면
- (x, y) => {
		return x + y;
	}
...
: 바로 리턴되는 경우는 return 생략 된다.
- (x, y) => x + y;

2. 실행해야 하는 함수 블록이 한 줄 이상이라면
- (x, y) => {
		const a = x + y;
		return a;
	}
...
3. 실행해야 하는 함수가 객체 리터럴을 return 하면?
- (x, y) => {
						return {a:x, b:y};
	}
...
- Rest 파라미터
1. 파라미터 ( … parameter) 함수 ( … 매개변수)

: … 매개변수는 []

: …매개변수는 arguments를 대체

1. Rest 파라미터 위치를 살펴보면
...
- 명칭에 대한 이해가 필수
    ...
    1. 요소 : 특정한 칸에 보관하는 값
    2. 인덱스 : 특정한 칸의 번호를 말함. 시작 부분부터 < 도착 전까지
    3. 길이 : length (칸의 갯수)
...
- 기능들
    ...
    : 원본 배열을 훼손하는 함수 (가능하면 사용하지 말자)

    : 원본을 유지하고(불변성 유지) 복사해서 결과를 돌려주는 함수 (비교대상이 필요하기 때문)

    function go (…arr){}

    […arr]

- 원본을 훼손하는 것과 원본을 유지하기 위해 복사하여 사용하는 이유
    - React의 Re-Rendering을 위해서

- 배열에서 고차함수들의 기본형을 꼭 알자
